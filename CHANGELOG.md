...
### Changed
- Dev dependency migrated from `@mariozechner/pi-coding-agent@0.73.0` to `@earendil-works/pi-coding-agent@0.74.0` following the library's npm scope change
...
### Changed
- Server no longer auto-starts on `session_start`. Use `/server start` and `/server stop` to manage it manually
- `/server` command supports `start`/`stop` argument autocomplete
- Errors (port in use, etc.) now surface as pi UI notifications instead of console output
